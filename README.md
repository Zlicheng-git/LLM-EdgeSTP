...
LLM-EdgeSTP 是首个面向船舶轨迹预测（Ship Trajectory Prediction, STP）任务、融合轨迹数据与海事文本知识、并支持边缘设备部署的大语言模型（LLM）框架。该系统通过跨模态推理，将连续的 AIS 轨迹与海事语义知识统一建模，显著提升了在复杂航行场景下的预测精度与工程实用性。
...
🧠 AIS-海事语义双模态协同推理框架
  利用轻量级 LLM（如 Gemma、LLaMA）作为推理引擎，融合原始海事提示与重构后的交互语义，生成自然语言形式的未来轨迹描述，并解码为精确坐标。
